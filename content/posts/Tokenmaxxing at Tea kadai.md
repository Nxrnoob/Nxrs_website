...
- now make a shortcut of the above flow so you dont type your tailscale ip again and again, paste this in your termux ````mkdir -p ~/.ssh cat > ~/.ssh/config <<EOF Host pc     HostName 100.x.y.z User your-username EOF chmod 600 ~/.ssh/config```` edit those lines with your username and ip address
...
- now in your phone termux run ssh pc (make sure the tailscale connection is on) and boom you are into your pc and to get the terminal session back just type ````tmux attach -t werk```` or type ````tmux ls```` which shows the running sesisons if you forget the name, and you got your claude code fable instance coding that subway surfers controller app using handgesture so you can post in LinkedIn.
...
Grass here touch it -> ![grass](/images/grass.jpg)